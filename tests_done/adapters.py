...
PAT = r"""'(?:[sdmt]|ll|ve|re)| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+""" # from https://github.com/openai/CLIP/blob/main/clip/simple_tokenizer.py
...
def run_multihead_self_attention(
    ...
    q = q.view(q.shape[:-1] + (num_heads,-1))# b , sq , nh , d_k // nh
    k = k.view(k.shape[:-1] + (num_heads, -1))
    v = v.view(v.shape[:-1] + (num_heads, -1))
...
def run_rope(
    ...
    # 创建频率
    freqs = 1.0 / (theta ** (torch.arange(0, d_k, 2).float() / d_k))  # (d_k // 2,) 构造 频率向量：torch.arange(0, d_k, 2) 得到 0,2,4…; 再除以 d_k；theta 通常是 10000.
...
class Tokenizer:
    def __init__(
        ...
        """
        self.vocab = vocab
        self.byte_to_token_id = {v: k for k, v in vocab.items()} # /explain byte_to_token_id:[bytes:token_id]
        self.merges = merges

        self.bpe_ranks = dict(zip(merges, range(len(merges))))# /explain bpe_ranks: {(<token1>, <token2>): rank}
    ...
    def encode(self, text: str) -> list[int]:
        ...
        sorted_special_tokens = sorted(self.special_tokens, key=len, reverse=True)# /explain sorted_special_tokens: ['<|endofprompt|>','<|endofchunk|>','<|endofresponse|>','<|startofchunk|>','<|startofprompt|>','<|startofresponse|>']
        pattern = "|".join(map(re.escape, sorted_special_tokens)) # /explain pattern: <\|endofprompt\|>|<\|endofchunk\|>|<\|endofresponse\|>|<\|startofchunk\|>|<\|startofprompt\|>|<\|startofresponse\|>
        if pattern:
    ...
    def encode_iterable(self, iterable: Iterable[str]) -> iter:
        ...
        for chunk in iterable:
            buffer += chunk
            # Process the buffer in a memory-efficient way
            while True:
                # Check if any special token appears in the buffer
                found_special = False
                for special in self.special_token_bytes:
                    idx = buffer.find(special.decode("utf-8"))
                    if idx != -1:
                        # Process everything before the special token
                        if idx > 0:
                            yield from self._tokenize_normal(buffer[:idx])
                        # Yield the special token's ID
                        yield self.byte_to_token_id[special]
                        # Keep the remaining buffer
                        buffer = buffer[idx + len(special):]
                        found_special = True
                        break
        ...
        if buffer:
            yield from self._tokenize_normal(buffer)
